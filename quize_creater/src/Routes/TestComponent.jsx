...
const CategoryQuestions = ({ question }) => {
 ...
 console.log(categories)
   ...
   return (
    ...
    <div key={index} className={`m-4 p-4 border flex-1`}  style={{ height: '300px' }} ref={index === 0 ? div1Drop : div2Drop}>
      
      <div id="div-1" className="m-4 p-4 border flex-1" ref={div1Drop}>
      <h1>{category[0].category}</h1>
          {categories.category1.map((buttonText, index) => (
            <div key={index}>{buttonText}</div>
          ))}
        </div>
        <div id="div-2" className="m-4 p-4 border flex-1" ref={div2Drop}>
        <h1>{category[1].category}</h1>
